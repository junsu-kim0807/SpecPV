...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model_path", type=str, required=True, help="Target model path or HF repo id.")
    p.add_argument("--draft_path", type=str, required=True, help="Draft model path or HF repo id.")
    p.add_argument(
        "--method",
        type=str,
        default="naive",
    ...
    for dataset in args.datasets:
        if dataset in ("aime2025", "codeelo"):
            ...
            if args.method == "specpv":
                cmd += ["--partial_length", str(args.partial_length), "--partial_spec_tokens", str(args.partial_spec_tokens)]
        else:
