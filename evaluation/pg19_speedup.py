...
def get_pred(rank, data, max_gen, device, model_path, draft_path, out_path, model_name, spec_config, method):
    device = torch.device(f'cuda:{rank}')
    model, tokenizer = load_model_and_tokenizer(model_path, draft_path, device)
    for json_obj in tqdm(data):
        # truncate to fit max_length (we suggest truncate in the middle, since the left and right side may contain crucial instructions)
        prompt = build_chat(tokenizer, json_obj['text'], model_name)
        ...
        with open(out_path, "a", encoding="utf-8") as f:
            ...
            f.write('\n')
        # avoid OOM
        del input, output, metrics, pred, prompt
...
if __name__ == '__main__':
    ...
    lengths = ['10k', '20k', '30k', '40k', '50k', '60k']
    for length in lengths:
        data = load_dataset('parquet', data_files=f'data/pg19_test/pg19_test_{length}.parquet')['train']
        # data = data.select(range(20))
        if not os.path.exists(f"outputs/{model_name}/pg19_speedup"):
            os.makedirs(f"outputs/{model_name}/pg19_speedup")
        out_path = f"outputs/{model_name}/pg19_speedup/pg19-{length}-{args.method}.jsonl"
        if args.method == "specpv":
            out_path = f"outputs/{model_name}/pg19_speedup/pg19-{length}-{args.method}-{args.partial_length}-{args.partial_spec_tokens}.jsonl"
        max_gen = 1024
        ...
        for rank in range(world_size):
            p = mp.Process(target=get_pred, args=(rank, data_subsets[rank], \
                        max_gen, device, args.model_path, args.draft_path, out_path, model_name, spec_config, args.method))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
